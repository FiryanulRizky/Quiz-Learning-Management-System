...
@section('breadcrump')
    ...
    <ol class="breadcrumb">
      ...
      <?php endif ?>
        <li class="active">Edit Data Soal Quiz</li>  
    </ol>
@stop
@section('content')
    <div class="row">
    ...
    <div class="box box-danger">
      <div class="box-header with-border">        
        <div class="pull-left"> 
          <h3 class="box-title">Form Edit Data Soal Quiz </h3>
        </div>   
        <div class="pull-right"> 
          <a href="{{{ action('Admin\SoalQuizController@index') }}}" class="btn btn-success btn-xs">
              <span class="glyphicon glyphicon-th-list" ></span> Daftar Soal 
          </a> 
          <a href="{{{action('Admin\SoalQuizController@detail', [$id_soal]) }}}" class="btn btn-primary btn-xs">
    ...
    <div style="display: block;" class="box-body"> 
      @if(Auth::user()->level  == 11)                        
        <form id="formQuizEdit" class="form-horizontal" role="form" method="POST" enctype="multipart/form-data" action="{{ url('admin/soal_quiz/'.$id_soal.'/simpanedit') }}">
      @elseif(Auth::user()->level  == 12)
        <form id="formQuizEdit" class="form-horizontal" role="form" method="POST" enctype="multipart/form-data" action="{{ url('trainer/soal_quiz/'.$id_soal.'/simpanedit') }}">
      @endif
            ...
            <div class="col-md-6">
              ...
              <div class="col-md-12">                    
                 <label for="id_quiz">ID Quiz</label>          
                  <select class="form-control " name="id_quiz">
                   <option value="{{$id_quiz}}">-- {{$id_quiz}} --</option>
                   @foreach ($quiz as $idQuiz)
                      <option value="{{ $idQuiz->id_quiz }}">id quiz : {{ $idQuiz->id_quiz }} | {{ $idQuiz->jenis_quiz }} | {{ $idQuiz->nama_modul }} </option>
                   @endforeach                                                                                 
                  </select>
                </div>
              </div>
          ...
          <div class="form-group"> 
             <div class="col-md-8 col-md-offset-5">
               <button type="submit" class="btn btn-primary" id="button-reg">
                ...
                <a href="{{{ action('Admin\SoalQuizController@detail', [$id_soal]) }}}" title="Cancel">
